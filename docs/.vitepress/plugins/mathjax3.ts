...
// todo: Since the Vitepress theme also needs to render mathematical formulas, it needs to be called globally in the component
// https://github.com/mathjax/MathJax-demos-node/blob/d9ba8c61e54683efc04d0e11d5812bc974da65db/preload/README.md
// https://github.com/mathjax/MathJax-demos-node/blob/d9ba8c61e54683efc04d0e11d5812bc974da65db/direct/README.md
...
function isValidDelim(state: StateInline, pos: number) {
  ...
  // check that the closing delimeter isn't followed by a number
  if (
    prevChar === 0x20 /* " " */ ||
    prevChar === 0x09 /* \t */ ||
...
function math_inline(state: StateInline, silent: boolean) {
  ...
  // This loop will assume that the first leading backtick cannot
  ...
  while ((match = state.src.indexOf("$", match)) !== -1) {
    ...
    // the first non escape when complete
    let pos = match - 1;
    while (state.src[pos] === "\\") {
      pos -= 1;
    }
  ...
  // Check for a valid closing delimiter
...
function math_block(
  ...
  for (next = start; !found; ) {
    ...
    if (pos < max && state.tShift[next] < state.blkIndent) {
      // a non-empty line with negative indent should stop the list:
      break;
    }
