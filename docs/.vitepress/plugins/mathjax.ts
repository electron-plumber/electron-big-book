...
function isValidDelim(state: StateInline, pos: number) {
  ...
  // check that the closing delimiter isn't followed by a number
  if (
    prevChar === 0x20 /* " " */ ||
    prevChar === 0x09 /* \t */ ||
...
function math_inline(state: StateInline, silent: boolean) {
  ...
  // This loop will assume that the first leading backtick can't
  // be the first character in state.src, which is known since
  // we've found an opening delimiter already.
  ...
  // Check if we have empty content, i.e.: $$.  Don't parse.
  if (match - start === 0) {
    if (!silent) {
      state.pending += "$$"
    }
